...
def match(datapath):
    df_tweets = pickle.load(open(datapath+"df_intweets.pkl", "rb"))
    ...
    tokenized_tweets = [[stemmer.stem(c) for c in tweet.split()] for tweet in list(tweets)]
    ...
    for i, t in enumerate(tokenized_tweets[:10]):
        ...
        ind = np.argpartition(news_scores, -400)[-400:]
        scores = [(i, news_scores[i]) for i in ind]
        # for score in enumerate(news_scores):
        #     if score[1] >= 0.4:
        #         scores.append(score)
        df_matched_news.loc[i] = [scores]

    df_matched_news.to_pickle(datapath + "news_match.pkl")
    df_matched_news.to_csv(datapath+"news_match.csv")
    #     hashes = hashtags[i]
    #     raw = raw_tweets[i]
