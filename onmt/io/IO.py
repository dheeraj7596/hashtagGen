...
def get_fields(data_type, n_src_features, n_conversation_features, n_tgt_features):
    """
    Args:
...
def load_fields_from_vocab(vocab, data_type="text"):
    ...
    n_conversation_features = len(collect_features(vocab, 'conversation'))
    fields = get_fields(data_type, n_src_features, n_conversation_features, n_tgt_features)
...
def build_my_dataset(fields, data_type, src_path, conversation_path, tgt_path, score_path, src_dir=None,
    ...
    if data_type == 'text':
        dataset = TextDataset(fields, src_examples_iter, conversation_examples_iter, tgt_examples_iter,
                              num_src_feats, num_conversation_feats, num_tgt_feats,
                              src_seq_length=src_seq_length,
...
def build_vocab(train_dataset_files, fields, data_type, share_vocab,
                ...
                conversation_vocab_path, conversation_vocab_size, conversation_words_min_frequency,
    ...
    for k in fields:
        counter[k] = Counter()
        # do not count the same src multi times
        last_ex[k] = ''
    ...
    for path in train_dataset_files:
        ...
        if logger:
            logger.info(" * reloading %s with size %d" % (path, len(dataset.examples)))
        for ex in dataset.examples:
            for k in fields:
                ...
                # add for kpg, Bad code
                if k in ['src', 'conversation']:
                    if last_ex[k] == ' '.join(val):
                        continue
                    last_ex[k] = ' '.join(val)
    ...
    if data_type == 'text':
        ...
        _build_field_vocab(fields["conversation"], counter["conversation"],
                           max_size=conversation_vocab_size,
                           min_freq=conversation_words_min_frequency)
        if logger:
            logger.info(" * conversation vocab size: %d." % len(fields["conversation"].vocab))
        ...
        if share_vocab:
            ...
            if logger:
                logger.info(" * merging src, conversation and tgt vocab...")
            merged_vocab = merge_vocabs(
                [fields["src"].vocab, fields["conversation"].vocab, fields["tgt"].vocab],
